...
func ForecastFunc(w http.ResponseWriter, r *http.Request) {
	...
	From(weather.WeatherForcast).
		GroupByT(func(item WeatherModel) string { // key
			splitDateTime := strings.Split(item.DateOfTemperature, " ")
			return splitDateTime[0]
		}, func(listWeather WeatherModel) WeatherModel { // value
			return listWeather
		}).
	...
	for i := 0; i < len(myList); i++ {
		...
		MaxTemp := From(myList[i].Group).
			SelectT(func(x WeatherModel) float64 { return x.Main.Temperature }).
			Max()
...
func GetForecastTest() ForecastModel {
